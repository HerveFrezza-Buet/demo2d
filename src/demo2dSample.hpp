...
namespace vq3 {
  namespace demo2d {
	...
	void check_empty() const {
	  if(empty)
	    throw bbox_exception("demo2d : empty bbox");
	}
	...
	BBox operator*(const Point& p) const {
	  ...
	  auto a = min & p;
	  auto b = max & p;
	  return {demo2d::min(a,b), demo2d::max(a,b)};
	}
	...
	BBox operator||(const BBox& bb) const {
	  ...
	  if(bb.empty)
	    return *this;
	  return {demo2d::min(min,bb.min), demo2d::max(max,bb.max)};
	}

	BBox operator&&(const BBox& bb) const {
	  if(!intersects(bb))
	    return BBox();
	  return {demo2d::max(min,bb.min), demo2d::min(max,bb.max)};
	}
      };
	...
	private:
	  RANDOM_DEVICE& rd;
	  double nb_samples_per_m2;
	  mutable std::vector<demo2d::Point> pts;
	  
	public:

	  using iterator = typename std::vector<demo2d::Point>::iterator;
